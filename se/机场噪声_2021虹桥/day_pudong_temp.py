...
class Mysqldb:
    ...
    def ins_to_tab(self, tab, values):
        ...
        :return:
        """
        sql = '''INSERT INTO {} ( pri, 点位, 日期, N1, N2, N3, Lamaxpb, Lwecpn, 记录时间 ) VALUES 
            (NULL, \'{}\', \'{}\', \'{}\', \'{}\', \'{}\', \'{}\', \'{}\', now())'''.format(
            tab, values[0], values[1], values[2], values[3], values[4], values[5], values[6])
...
def count_hb(sht):
    ...
    for i in range(3, 1823):
        ...
        if jx := sht.range('h{}'.format(i)).value not in [None, '起降']:
            if 7 <= hour < 19:
                hb['day'] += 1
            elif 19 <= hour < 22:
                hb['dust'] += 1
...
def cal_lwecpn_day(lamaxpb, hb):
    res=lamaxpb + 10 * math.log10(hb['day'] + 3 * hb['dust'] + 10 * hb['night']) - 27
    return round(res,1)
...
def run_oneday(walkpath):
    ...
    for file24name, sht in yieldsht(walkpath):
        ...
        hb = count_hb(sht)
        lwecpn = cal_lwecpn_day(lamaxpb, hb)
        sumresult = [dw := (lambda x: re.search(r'(\d{1,2})#(\d{2,4})', x).group(1))(file24name),
                     date := (lambda x: re.search(r'(\d{1,2})#(\d{2,4})', x).group(2))(file24name),
                     hb['day'], hb['dust'], hb['night'], lamaxpb, lwecpn]
        db.ins_to_tab('day', sumresult)  # 写入lepnk行


if __name__ == '__main__':
    run_oneday(r"C:\Users\Administrator\Desktop\噪声桌面\复算")
