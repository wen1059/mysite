...
def gendic(sht):
    ...
    def gendic_inner(dic):
        ...
        # maxlap = maxla + 10 * math.log10(td * 2 / 20)  # lamax和td计算得到的l’amax ,这里的td是te懒的改名字
        maxlap = lepn-13  # 更新，由maxlap改为lepn，变量名没有改。
        if eli == 1:
            if qj == '起飞':
...
def gendic_addmax(dic):
    ...
    for jx in dic:
        ...
        for hl in dic[jx]:
            ...
            max_nor = math.fsum(nor_lst) / len(nor_lst) if nor_lst else 0  # 第三层，某机型，某航路的max
            dic[jx][hl].setdefault('max_q', max_nor)
            if max_nor:
                jxmaxlst.append(max_nor)  # 添加到列表，用来算第二层max
                dicmaxlst.append(max_nor)  # 添加到列表，用来算第一层max
        max_jx = math.fsum(jxmaxlst) / len(jxmaxlst) if jxmaxlst else 0
        dic[jx].setdefault('max_q', max_jx)
    max_dic = math.fsum(dicmaxlst) / len(dicmaxlst) if dicmaxlst else 0
...
def main():
    ...
    while True:
        try:
            run_oneday_week(r'C:\Users\Administrator\Desktop\虹桥噪声桌面\周平均自动计算_虹桥_精密_lamax', 'day虹桥_精密_lamax', 'week虹桥', app_24)
        except Exception as e:
            print(e)
        time.sleep(3)
    # app_24.kill()
