    ...
    for jx in dic:
        ...
        for hl in dic[jx]:
            ...
            if dic[jx][hl]['eli_q']:
                ...
                print(jx, ',', hl, ',', '起飞', ',', max_lp_q, ',',lamaxp_part1)  # 为了输出替换值
            if dic[jx][hl]['eli_j']:
                lamaxp_all.extend([max_lp_j] * len(dic[jx][hl]['eli_j']))
                print(jx, ',', hl, ',', '降落', ',', max_lp_j, ',',lamaxp_part2)
    return lamaxp_all
...
def main():
    ...
    while True:
        try:
            run_oneday_week(r"C:\Users\Administrator\Desktop\新建文件夹 (2)", 'day虹桥_精密', 'week虹桥', app_24)
        except Exception as e:
            print(e)
        time.sleep(3)
    # app_24.kill()
