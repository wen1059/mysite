...
def gendic_addmax(dic):
    ...
    for jx in dic:
        jxmaxlst = []  # 第二层，某机型max
        for hl in dic[jx]:
            nor_lst = dic[jx][hl]['nor']
            max_nor = math.fsum(nor_lst) / len(nor_lst) if dic[jx][hl]['nor'] else 0  # 第三层，某机型，某航路的max
            dic[jx][hl].setdefault('max', max_nor)
            jxmaxlst.append(max_nor)  # 添加到列表，用来算第二层max
        max_jx = math.fsum(jxmaxlst) / len(jxmaxlst)
        dic[jx].setdefault('max', max_jx)
        dicmaxlst.append(max_jx)  # 添加到列表，用来算第一层max
    max_dic = math.fsum(dicmaxlst) / len(dicmaxlst)
    dic.setdefault('max', max_dic)
    return dic
...
def main():
    ...
    while True:
        try:
            run_oneday_week(r'C:\Users\Administrator\Desktop\虹桥噪声桌面\周平均自动计算_虹桥_简易', 'day虹桥_简易', 'week虹桥', app_24)
        except Exception as e:
            print(e)
        time.sleep(3)
    # app_24.kill()
