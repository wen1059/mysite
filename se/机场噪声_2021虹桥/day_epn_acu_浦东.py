...
def gen_list(sht):
    ...
    for i in range(3, 2080):
        ...
        if jx == '机型':  # 更新背景值，背景值在k列“机型”字段的上两行
            bg = float(sht.range('m4').value)  # sht.range('k{}'.format(i - 2)).value # 4.17更新，背景只取k4格
            eli = sht.range('o{}'.format(i - 2)).value
            continue
        if str(jx).isdigit():
            jx = str(int(jx))  # 机型转为整数字符串,因为xlwings读出来的数字是浮点数
        hl = sht.range('m{}'.format(i)).value
...
if __name__ == '__main__':
    ...
    while True:
        try:
            run_oneday_week(r'C:\Users\Administrator\Desktop\虹桥噪声桌面\周平均自动计算_浦东', 'day浦东_精密',
                            'week浦东')
        except Exception as e:
            print(e)
        time.sleep(3)
