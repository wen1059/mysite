...
def read_dd(file: str | os.PathLike):
    """
    读取钉钉考勤表
    ...
    df['打卡时间'] = df[['打卡时间', '打卡结果', '打卡地址']].apply(
        lambda x: ''.join(x[['打卡时间', '打卡地址']]) if x['打卡结果'] == '外勤' else x['打卡时间'],
        axis=1)  # 如果是外勤，时间后面加上地址
    ...
    df_out = pd.DataFrame(dict_temp).sort_index().T.sort_index()
    # print(df_out.columns)
    return df_out


def write_dd(df: pd.DataFrame, file: str | os.PathLike):
    """
    将转换好的df写入到excel
    :param df:
    :param file:
