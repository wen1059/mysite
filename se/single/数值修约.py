...
def epa_rounding(
    ...
    """
    四舍六入五成双保留指定有效位数，同时满足不超过检出限有效位数。
    ...
    zoom = mdl if zoom > mdl else zoom  # 根据检出限修正，达到这个效果：测定结果小于xxx时，结果保留小数和检出限一致。

    # 放大10**zoom次方后的数字
    num_zoomed = num * (10 ** zoom)
    # 放大倍数后的数字四舍六入修约到整数
    num_zoomed_rounded = float(Decimal(f'{num_zoomed}').quantize(Decimal('0'), decimal.ROUND_HALF_EVEN))
    # 缩小倍数到原来。
    num: float = num_zoomed_rounded / (10 ** zoom)
    # 整数的话取整去除x.0，小数的话补全后面的0。
    return str(int(num)) if num == int(num) else f"{num: .{zoom}f}"
