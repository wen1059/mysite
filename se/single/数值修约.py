...
def epa_rounding(
    ...
    放大到10^i次方倍，如保留3位有效数字就把数字放大到xxx.______
    修约到整数，再除以放大倍数
    :param num: 要修约的数字
    :param digit: 有效位数
    :param mdl: 检出限小数位数
    ...
    if int(num) > 0:
        ...
        behind_0 = rst if (rst := re.search(r'\.(0*)', str(num)).group(1)) else ''  # 小数点后面的0
    ...
    num_zoomed_rounded = float(
        Decimal(f'{num_zoomed}').quantize(Decimal('0'), decimal.ROUND_HALF_EVEN))  # 放大倍数后的数字四舍六入修约到整数
