...
class DrawGradient:
    def __init__(self, size=(1024, 1024)):
        self.im = Image.new('RGB', size, None)
    ...
    def put_scaled_pixel(self, x, y, basecolor):
        ...
        # distance = max(abs(w / 2 - x), abs(w / 2 - y))  # 绘制正方形扩散
        percentage = distance / (self.im.size[0] / 2)  # 距离和半径的比值，用来判断rgb值渐变的百分比，离圆心越远，颜色越淡
        color_scaled = tuple(int(i + (255 - i) * percentage) for i in basecolor)  # 根据到圆心的距离计算到的渐变过的rgb值
