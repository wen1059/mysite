"""
新的意见稿方式计算。使用了“day_epn_acu_虹桥.py”作为模板，临时方案，后期可使用pandas重构。
改了一半，不改了，用pandas重写，之前太乱了，又是替换又是大于10db的。
"""
import math
import os
import re
...
def count_hb(sht):
    ...
    for i in range(3, 2080):
        if sht.range('a{}'.format(i)).value == '监测开始时间':
            ...
            # print(hour)
        if sht.range('f{}'.format(i)).value in [None, 'LAE(10)']:  # 此处修改过，值对应LAE列,如果有事件，统计架次
            continue
...
def gen_list(sht):
    ...
    for i in range(3, 2080):  # 一天的行数
        ...
        lepn = sht.range('f{}'.format(i)).value  # 改为LAE,但名字还是lepn
        maxla = sht.range('h{}'.format(i)).value
        td = sht.range('e{}'.format(i)).value
...
if __name__ == '__main__':
    while True:
        run_oneday_week(r'\\10.1.78.254\环装-实验室\实验室共享\2023鸡场\__投递到这里自动计算__', '新标准计算',
                        'week虹桥')
        time.sleep(3)
